...
describe('when there is initially one user at db', () => {
  ...
  test('creation fails with proper statuscode and message if password is too short or missing,', async () => {
    ...
    expect(result.body.error).toContain("password has to be minimum of 3 characters")

    const usersAtEnd = await helper.usersInDb()
    expect(usersAtEnd).toHaveLength(usersAtStart.length)
  })
  test('creation fails with proper statuscode and message if username is too short or missing,', async () => {
    const usersAtStart = await helper.usersInDb()
